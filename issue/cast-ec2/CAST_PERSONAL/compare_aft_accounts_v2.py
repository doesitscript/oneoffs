#!/usr/bin/env python3
"""
Script to compare AFT account requests with actual AWS accounts using AWS profiles.
This script extracts account request data from Terraform files and compares
with the actual accounts by parsing AWS profile names and account IDs.
"""
...
class AFTAccountComparator:
    ...
    def get_aws_profiles_accounts(self) -> List[Dict[str, Any]]:
        """Get AWS accounts from profile names."""
        print("🔍 Extracting AWS accounts from profile names...")
        ...
        try:
            ...
            for profile in profiles:
                # Parse profile name to extract account info
                # Format: AccountName_AccountID_admin
                parts = profile.split('_')
                if len(parts) >= 3 and parts[-1] == 'admin':
                    account_id = parts[-2]
                    account_name = '_'.join(parts[:-2])  # Everything except last 2 parts
                    
                    accounts.append({
                        "id": account_id,
                        "name": account_name,
                        "profile": profile,
                        "email": f"AwsAccount+{account_name}@breadfinancial.com"  # Estimated
                    })
            
            print(f"✅ Found {len(accounts)} AWS accounts from profiles")
            return accounts
            
        except Exception as e:
            print(f"❌ Error getting AWS profiles: {e}")
            return []
    
    def compare_accounts(self) -> Dict[str, Any]:
        """Compare AFT requests with actual AWS accounts."""
        print("🔄 Comparing AFT requests with AWS profile accounts...")
        
        aws_accounts = self.get_aws_profiles_accounts()
        ...
        for request in self.account_requests:
            ...
            # Try to match by name first (normalize names)
            normalized_aft_name = self._normalize_name(account_name)
            for aws_name, aws_account in aws_by_name.items():
                normalized_aws_name = self._normalize_name(aws_name)
                if normalized_aft_name == normalized_aws_name:
                    ...
                    matched = True
                    del aws_by_name[aws_name]  # Remove from unmatched
                    break
            
            # Try to match by email if name didn't match
            if not matched and account_email in aws_by_email:
    ...
    def print_comparison_report(self, comparison: Dict[str, Any]):
        ...
        print("\n" + "="*80)
        print("📊 AFT ACCOUNT REQUEST vs AWS PROFILE ACCOUNTS COMPARISON REPORT")
        print("="*80)
        ...
        # Show matched accounts (first 10)
        if comparison["matched_accounts"]:
            print(f"\n✅ MATCHED ACCOUNTS (showing first 10):")
            for match in comparison["matched_accounts"][:10]:
                aft_name = match["aft_request"]["control_tower_parameters"].get("AccountName", "")
                aws_name = match["aws_account"]["name"]
                aws_id = match["aws_account"]["id"]
                match_type = match["match_type"]
                print(f"   {aft_name} → {aws_name} ({aws_id}) [{match_type}]")
...
def main():
    ...
    output_file = "/Users/a805120/develop/oneoffs/issue/cast-ec2/CAST_PERSONAL/aft_account_comparison_v2.json"
