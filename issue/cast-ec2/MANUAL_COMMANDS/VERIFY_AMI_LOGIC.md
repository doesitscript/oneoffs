# Verify AMI Selection Logic

Run this command to verify Terraform logic matches expected AMI `ami-08acfbd98127249e6`:
...
aws ec2 describe-images \
  ...
  --query 'Images[*].{AMI_ID:ImageId,ARN:Tags[?Key==`Ec2ImageBuilderArn`].Value | [0]}' \
...
if parsed:
    ...
    print('Highest Version/Build AMI:')
    print(json.dumps({
    ...
    print(f"Match: {'✅ YES' if highest['AMI_ID'] == 'ami-08acfbd98127249e6' else '❌ NO'}")
    
    if len(parsed) > 1:
        print(f"\nAll {len(parsed)} winserver2022 AMIs (sorted):")
...
This verifies:
1. Filters for `Ec2ImageBuilderArn` tag
2. Filters for `winserver2022` in ARN
3. Extracts version/build from ARN
4. Sorts by highest version, then highest build
5. Returns the selected AMI ID

Expected result: `ami-08acfbd98127249e6`
