...
# Focused AWS CIDR Usage Verification Script
# Compares IPAM allocations against actual VPC usage in us-east-2 and us-west-2 only
...
FOCUS_REGIONS=("us-east-2" "us-west-2")
...
# Step 2: Scan focused regions only
scan_focused_regions() {
    log "Step 2: Scanning focused regions: ${FOCUS_REGIONS[*]}"
    ...
    log "Found $(wc -l < "$profiles_file") AWS profiles to scan"
    ...
    while read -r profile; do
        ...
        # Scan each focused region
        for region in "${FOCUS_REGIONS[@]}"; do
    ...
    log "VPC scan completed. Found $(wc -l < "$vpc_file") VPCs in focused regions."
}
...
# Main execution
main() {
    log "Starting Focused AWS CIDR Usage Verification"
    log "Output directory: $OUTPUT_DIR"
    log "Focused regions: ${FOCUS_REGIONS[*]}"
    
    # Step 1: Scan IPAM allocations
    scan_ipam_allocations
    
    # Step 2: Scan focused regions
    scan_focused_regions
