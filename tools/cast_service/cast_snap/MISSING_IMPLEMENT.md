x Missing attributes (compared to main() function):
x keep: int (default 0)
x tag: Optional[List[str]]
x copy_instance_tags: Optional[List[str]]
x copy_volume_tags: Optional[List[str]]
Additional issues:
